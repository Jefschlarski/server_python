...
from sqlalchemy import create_engine, Column, Float, Boolean, Double, Integer, String, Sequence
...
def add_Ordem(ordem):
    Session = sessionmaker(bind=engine)
    session = Session()
    nova_ordem = OrdemServico(title=ordem.title,geojson=ordem.geojson,lat=ordem.lat,lng=ordem.lng,relator=ordem.relator,date=ordem.date,status=ordem.status)
